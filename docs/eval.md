...
| EV-006 | Ollama (if running) responds to `curl http://localhost:11434/api/tags` | Medium | ✅ PASS / ❌ FAIL / ⬜ SKIP | |
...
| EV-131 | Ollama provider connects and gets response from local model | Critical | ✅ PASS / ❌ FAIL | |
| EV-132 | LocalAI provider works as fallback when Ollama is down | High | ✅ PASS / ❌ FAIL / ⬜ SKIP | |
...
| IT-007 | **Error recovery chain** | P6, P7 | Kill Ollama → Submit application → Verify fallback to LocalAI → Restart Ollama → Verify normal operation | System recovers without manual intervention |
